...
Find_Roots (void)
{
  ...
  for (r = 1; r < 256; r++) {
	...
	if (DEBUG_RS) fprintf(stderr, "Root found at r = %d, (255-r) = %d\n", r, (255-r));
      }
  }
}
			 ...
			 int erasures[])
{
  ...
  if ((NErrors <= NPAR) && NErrors > 0) { 
	...
	if (DEBUG_RS) fprintf(stderr, "Error loc i=%d outside of codeword length %d\n", i, csize);
	return(0);
      }
    }
	...
	denom ^= gmult(Lambda[j], gexp[((255-i)*(j-1)) % 255]);
      ...
      if (DEBUG_RS)fprintf(stderr, "Error magnitude %#x at loc %d\n", err, csize-i);
    ...
    if (DEBUG_RS && NErrors) fprintf(stderr, "Uncorrectable codeword\n");
    return(0);
  }
}
